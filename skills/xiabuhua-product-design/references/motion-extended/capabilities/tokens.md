# 三层 token、fluid type/space、duration/easing/spring/stagger、transform budget
...
CSS transform/opacity plus chosen controller

## 版式与样式

token hierarchy controls distance/easing/stagger

## 时序与轨道

source token values are project parameters

## 取消与资源管理

controller owns cancellation and will-change cleanup

## 降级

reduced maps to instant/short fade

## 执行步骤

只移植语义和预算，数值按渲染调参，不全局强制

## 验收

token 对应状态/层级；reduced 保语义；will-change 有生命周期
