# perspective camera、occlusion、sort、contact shadow、colour temperature、DOM overlay
...
perspective planes plus DOM text

## 版式与样式

occlusion/sort/shadow establish depth

## 时序与轨道

camera/world tracks map to scroll or pointer

## 取消与资源管理

pause/dispose scene resources

## 降级

static collage/DOM fallback on unsupported

## 执行步骤

明确 spatial narrative 才启用，DOM 可读/lazy/visibility/fallback 先行

## 验收

遮挡/接触影/投影/对齐可测；移动/reduced/无 WebGL/读屏成立
