# screen-window、cooperative scan、batched DOM、dispose/cache、texture slicing、audio streaming
...
resource owner creates heavy layer

## 版式与样式

active window biased forward; sliced texture preserves geometry

## 时序与轨道

cooperative scan yields between objects

## 取消与资源管理

dispose on exit and cache by URL

## 降级

low-memory/static/DOM fallback

## 执行步骤

窗口/资源计数先接 graphics adapter，音频另立项

## 验收

进出窗口创建/销毁可观察，无资源泄漏，隐藏标签可恢复
