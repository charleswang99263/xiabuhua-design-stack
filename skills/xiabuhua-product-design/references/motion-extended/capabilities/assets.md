# own-background keying、alpha bbox、line art、cx/baseline/crown、sprite、resolution/density、provenance
...
```json
{
  ...
  "runtime_required": "artifact pipeline",
...
alpha image/sprite/line art

## 版式与样式

bbox/cx/baseline/crown inform placement

## 时序与轨道

load/decode before reveal; no empty margin jump

## 取消与资源管理

abort/retry asset load

## 降级

fallback asset or text description

## 执行步骤

复制字段和检查门，不复制图片/字体；元素记录来源和预算

## 验收

透明边界、密度、许可、尺寸、替换策略可核验
