# SVG goo/displacement、Houdini、OffscreenCanvas worker、pattern buffer、DPR
...
```json
{
  ...
  "runtime_required": "graphics + tactile",
...
SVG/CSS paint/offscreen canvas

## 版式与样式

texture tile or filter bounds cannot alter content geometry

## 时序与轨道

worker/render loop pauses when hidden

## 取消与资源管理

terminate worker and release bitmap

## 降级

static grain/filter-free fallback

## 执行步骤

测得主线程瓶颈才启用；静态 fallback 先行

## 验收

filter/worker 失败可回退；DPR/visibility/keyboard/reduced 通过
