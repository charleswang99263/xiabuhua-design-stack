# 20 个 recipe 的逐条 route、机制、输入和 oracle
...
```json
{
  ...
  "runtime_required": "interaction + museum",
...
recipe-specific local layer

## 版式与样式

geometry/style comes from host direction

## 时序与轨道

source timing/easing must be read per recipe

## 取消与资源管理

recipe defines reversal/cancel/dispose

## 降级

reduced/static behavior per recipe

## 执行步骤

按下方 20 条逐条选择，不将无 recipe 能力强套

## 验收

每条有 job/state/input/reduced/cancel/dispose/perf/oracle
