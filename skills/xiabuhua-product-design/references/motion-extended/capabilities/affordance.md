# invitation window、painted bbox、down/up、handoff、auto-demo ownership、numeric oracle
...
```json
{
  ...
  "runtime_required": "interaction + tactile",
...
painted pixels plus hit region

## 版式与样式

mark follows painted bbox, not box

## 时序与轨道

invitation window covers dwell

## 取消与资源管理

down records; up decides click vs drag

## 降级

keyboard/touch path and static mark remain

## 执行步骤

维持 partial；逐机制补 invitation、coverage、real input 检查

## 验收

dwell 期间可发现；透明边界不吞命中；gesture/keyboard 可达
