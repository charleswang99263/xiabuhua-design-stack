...
These are optional visual references for HTML Deck Runtime, not a default style
picker or a source of approval. Use them only when they fit the brief or an
approved/inherited direction. A preset may suggest typography, color, layout,
material, or component syntax; it cannot override the selected specimen or any
locked agreement field. Abstract shapes are examples, not a ban on supplied or
direction-appropriate imagery.

**Execution handoff:** Fixed-stage and runtime constraints live in [`html-deck-runtime`](../../../html-deck-runtime/SKILL.md) and [`viewport-base.css`](../../../html-deck-runtime/viewport-base.css). This document is a visual reference only.
...
## Diagnostic prompts, not global bans

The following patterns can become generic when used without a reason. They are
diagnostic prompts only: a confirmed direction may use any font, color, layout,
or material when the user, content, audience, and delivery context support it.

- Repeated font, color, layout, or card-grid choices without a content reason.
- Purple gradients, solid colors, system fonts, or illustrations used by habit.
- Glass, shadow, motion, or texture that does not clarify hierarchy or state.
