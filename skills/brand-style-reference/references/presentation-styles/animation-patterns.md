...
Use this reference when generating presentations. Match animation to the
approved/inherited motion language, narrative role, content density, and
runtime budget. These are optional patterns, not a motion preset or a reason to
add effects to a quiet/static direction.
...
```css
/* Optional gradient mesh — use only when the confirmed material supports it */
.gradient-bg {
    background:
